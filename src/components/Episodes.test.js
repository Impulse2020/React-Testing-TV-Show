...
test("no episodes when the episodes prop  is an empty array", () =>{
    const {queryAllByTestId} = render(<Episodes episodes={[]} />);
    expect(queryAllByTestId("episodes").length).toBe(0);

})

test("The episodes component properly maps an array", () => {
    const {queryAllByTestId} = render(<Episodes episodes={episodes.data} />);
    expect(queryAllByTestId("episodes").length).toBe(2);
})